...
async function showComment(pageNum){
  ...
  for(var i in titleRes.result){
    if(titleRes.result[i].id == pageNum){
      if(i>0){
        let title = '';
        if(titleRes.result[i-1].title.length>25) title = titleRes.result[i-1].title.substring(0,21)+'...'
        else title = titleRes.result[i-1].title;

        titleHtml += `
        <div class="notice-tab" id="notice-next-tab" onclick="location.href = '/notice/${titleRes.result[i-1].id}'">
          <span class="tab-title">다음글</span>
          <span class="tab-content-title">${title}</span>
        </div>
        `
      }
      if(Number(i)+1<titleRes.result.length){
        let title = '';
        if(titleRes.result[Number(i)+1].title.length>25) title = titleRes.result[Number(i)+1].title.substring(0,21)+'...'
        else title = titleRes.result[Number(i)+1].title;
        titleHtml += `
