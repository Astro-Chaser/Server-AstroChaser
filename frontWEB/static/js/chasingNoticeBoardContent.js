...
async function showContent(pageNum){ 
    const contentRes = await getAPI(hostAddress, `app/notice/chasing/${pageNum}`);
    if(contentRes.isSuccess==false) location.href = '/chasing/notice'
    let html = `
...
async function showComment(pageNum){
  ...
  const titleRes = await getAPI(hostAddress, 'app/notice/title/chasing');
  ...
  for(var i in titleRes.result){
    if(titleRes.result[i].id == pageNum){
      if(i>0){
        titleHtml += `
        <div class="notice-tab" id="notice-next-tab" onclick="location.href = '/chasing/notice/${titleRes.result[i-1].id}'">
      ...
      if(Number(i)+1<titleRes.result.length){
        titleHtml += `
        <div class="notice-tab" id="notice-prev-tab" onclick="location.href = '/chasing/notice/${titleRes.result[Number(i)+1].id}'">
